...
class XLSXExportRow(models.AbstractModel):
    _name = "xlsx.export.row"
    ...
    @api.model
    def _get_line_vals(self, records, line_field, fields):
        ...
        for field in fields:
            ...
            if "/" in raw_field:
                if not line_field:
                    line_field = raw_field.split("/", 1)[0]
                inline = True
                raw_field = raw_field.split("/", 1)[1]

                # raw_field.replace("/", ".")
                # pair_inline_fields.append((field, raw_field.split("/", 1)[1]))
            ...
            # --
            pair_fields.append((field, raw_field, inline))
        for line in lines:
            ...
            for field in pair_fields:  # (field, raw_field)
                ...
                if field[2]:
                    if not set_inline:
                        # self._set_inline_vals(line, line_field, pair_inline_fields, vals, field_cond_dict, style_cond_dict, field_style_dict)
                        self._set_inline_vals(line, line_field, pair_fields, vals, field_cond_dict, style_cond_dict, field_style_dict)
                        set_inline = True
                    continue
    ...
    @api.model
    def _set_inline_vals(self, record, line_field, pair_fields, vals, field_cond_dict, style_cond_dict, field_style_dict):
    ...
    @api.model
    def _fill_workbook_data(self, workbook, records, data_dict):
        """ Fill data from record with style in data_dict to workbook """
        if not records or not data_dict:
            return
        try:
            for sheet_name in data_dict:
                ...
                self._fill_lines(ws, st, records)
    ...
    @api.model
    def _fill_lines(self, ws, st, records):
        line_fields = list(ws)
        ...
        for line_field in line_fields:
            ...
            vals, func = self._get_line_vals(records, line_field, fields)
    ...
    def export_xlsx(self, template, res_model, res_ids):
        ...
        ptemp = ConfParam.get_param("path_temp_file") or "."
        ...
        os.remove(ftemp)
        self._fill_workbook_data(wb, records, export_dict)
        # Return file as .xlsx
        content = BytesIO()
        wb.save(content)
