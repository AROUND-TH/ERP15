...
class XLSXImportRow(models.AbstractModel):
    _name = "xlsx.import.row"
    ...
    @api.model
    def _post_import_operation(self, template, records, operation):
        """ Run python code after import """
        if not records or not operation:
            return
    ...
    def import_xlsx(self, import_file, template, res_model=False, res_id=False):
        ...
        records = self._import_record_data(import_file, record, data_dict["__IMPORT__"])
        # Post Import Operation, i.e., cleanup some data
        if data_dict.get("__POST_IMPORT__", False):
            self._post_import_operation(template, records, data_dict["__POST_IMPORT__"])
        return records
