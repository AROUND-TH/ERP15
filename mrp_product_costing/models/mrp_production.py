...
class MrpProduction(models.Model):
    ...
    # standard unit costs
    std_prod_cost = fields.Float('Current Standard Cost', digits='Product Price', compute='calculate_std_prod_cost', store=True, group_operator="avg")
    ...
    @api.depends('product_id','ovh_product_cost','ovh_components_cost')
    def calculate_std_prod_cost(self):
        for production in self:
            # @Sum with Overhead Costs.
            # production.std_prod_cost = production.product_id.standard_price
            production.std_prod_cost = production.product_id.basic_cost + production.ovh_product_cost + production.ovh_components_cost
        return True
    ...
    @api.depends('workorder_ids','state','product_id','product_uom_qty', 'workorder_ids.duration_expected')
    ...
    def _calculate_planned_costs(self):
        for production in self:
            plannedvaramount = plannedfixedamount = duration_expected_working = 0.0
    ...
    def calculate_actual_by_product_amount(self):
        ...
        for production in self:
            ...
            # @Set by_product_amount to zero.
            production.by_product_amount = 0.0
            production.by_product_amount_unit = 0.0

            # @Disable calculation of by_product_amount.
