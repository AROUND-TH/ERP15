...
class ReceiptBillingCustomer(models.Model):
    _name = 'receipt.billing.customer'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _description = "Receipt Customer Bill"
    ...
    line_ids = fields.One2many('receipt.billing.customer.line', 'receipt_id', 
        string='Bill Items', 
        readonly=False, 
        states=READONLY_STATES,
        # copy=True,
    ...
    #         for node in doc.xpath("//button[@name='button_open_invoices']"):
    ...
    def _compute_journal_id_domain(self):
        for rec in self:
            journal_config = self.env['select.journal.config'].search([('form_select', '=', 'receipt_billing_customer')])
    ...
    def _compute_name(self):
        for receipt in self:
            if receipt.state == 'draft' or not receipt.move_id:
                receipt.name = "/"
            else:
                receipt.name = receipt.move_id.name
    ...
    def _compute_amount(self):
        for receipt in self:
            ...
            receipt.line_count = count
            receipt.amount_total = total

    @api.depends('amount_total', 'company_currency_id')
    def _compute_amount_text(self):
        for receipt in self:
            receipt.amount_total_text = receipt.company_currency_id.amount_to_text(receipt.amount_total, "th_TH", "th")


    @api.depends('partner_id')
    def _compute_company_id(self):
        for receipt in self:
            receipt.company_id = receipt.company_id or self.env.company
        ...
        for receipt in self:
            ...
            for line in receipt.line_ids:
                if line.balance >= 0:
                    ...
                    debit = 0.0
                    credit = (-1) * line.balance
                vals.append(
                    (0, 0,
                        {
                            'account_id': receipt.bank_account_id.account_code.id,
                            'partner_id': receipt.partner_id.id,
                            'name': line.name,
                            'currency_id': receipt.company_currency_id.id,
                            'debit': debit,
                            'credit': credit,
                        }
                    )
                ...
                for deduct in line.deduct_ids:
                    if deduct.deduct_amount >= 0:
                        debit = deduct.deduct_amount
                        credit = 0.0
                    else:
                        debit = 0.0
                        credit = (-1) * deduct.deduct_amount
                    vals.append(
                        (0, 0,
                            {
                ...
                vals.append(
                    (0, 0,
                        {
                            'account_id': receipt.partner_id.property_account_receivable_id.id,
                            'partner_id': receipt.partner_id.id,
                            'name': line.name,
                            'currency_id': receipt.company_currency_id.id,
                            'debit': debit,
    ...
    def button_cancel(self):
        for receipt in self:
            # @Revise to force cancel 'Journal Entry' and this 'Receipt Customer Bill'
            if receipt.move_id:
                if receipt.move_id.state == 'posted':
                    # raise UserError(_("Cannot cancel 'Receipt Customer Bill' that 'Journal Entry' already in Posted status."))
                    receipt.move_id.button_draft()
    ...
    def _compute_stat_buttons_from_reconciliation(self):
        ...
        self._cr.execute('''
            SELECT
                ...
                invoice.move_type
            FROM receipt_billing_customer payment
        ...
        for pay in self:
            ...
            # @Set line.bill_id.is_paid = False
            if pay.reconciled_invoices_count == 0:
    ...
    @api.depends('state', 'reconciled_invoices_count')
    def _compute_reconcile_state(self):
        for pay in self:
            if pay.state == 'done':
                if pay.reconciled_invoices_count > 0:
                    pay.reconcile_state = 'reconciled'
                    pay.reconcile_status = '%s Invoice' %(pay.reconciled_invoices_count)
                else:
    ...
    #     default['name'] = "/"
    #     return super(ReceiptBillingCustomer, self).copy(default)

    def unlink(self):
        for record in self:
            if record.state == 'done':
                raise UserError(_("Cannot delete 'Receipt Customer Bill' that 'Status' in 'Done' status."))
        moves = self.with_context(force_delete=True).move_id
        res = super(ReceiptBillingCustomer, self).unlink()
        moves.unlink()
        return res



class ReceiptBillingCustomerLine(models.Model):
    _name = 'receipt.billing.customer.line'
    _description = "Receipt Customer Bill Line"
    _order = 'receipt_name, id'
    _check_company_auto = True


    receipt_id = fields.Many2one('receipt.billing.customer', 
        string='Receipt Customer Bill',
        index=True, 
        readonly=True, 
        # required=True, 
        auto_join=True, 
    ...
    receipt_name = fields.Char(string='Number', related='receipt_id.name', store=True, index=True)
    ...
    bill_id = fields.Many2one('account.billing.customer', 
        string='ST No.',
        index=True, 
        required=True, 
    ...
    company_id = fields.Many2one(related='receipt_id.company_id', 
        store=True,
        readonly=True,
    ...
    deduct_ids = fields.One2many('receipt.billing.customer.deduct', 'line_id', 
        string='Deduction Items', 
        copy=True, 
    )

    _sql_constraints = [
        ('key_unique', 'UNIQUE(receipt_id, bill_id)', _("'ST No.' must be unique in Bill Line !")),
    ]
    ...
    def _compute_bill_id_domain(self):
        for line in self:
            receipt_partner_id = line.receipt_id.partner_id.id or 0
            bill_ids = self.env['account.billing.customer'].search([('partner_id','=',receipt_partner_id),('state','=','done'),('is_paid','=',False)])
...
class ReceiptBillingCustomerDeduct(models.Model):
    _name = 'receipt.billing.customer.deduct'
    _description = "Receipt Customer Bill Deduct"


    line_id = fields.Many2one('receipt.billing.customer.line', 
        string='Receipt Customer Bill Line',
        index=True, 
        readonly=True, 
        required=True, 
        ondelete="cascade",
