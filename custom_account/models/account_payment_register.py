...
class AccountPaymentRegister(models.TransientModel):
    ...
    is_checks = fields.Boolean(compute='_compute_is_checks', string='Is Checks')

    @api.depends("payment_method_line_id")
    def _compute_is_checks(self):
        for rec in self:
            if rec.payment_method_line_id.name == 'Checks':
                rec.is_checks = True
            else:
                rec.is_checks = False

    # @Override method _create_payments
    def _create_payments(self):
        ...
        if self.bank_account_id:
            for payment in payments:
                payment.bank_account_id = self.bank_account_id
                if not payment.bank_account_id.account_code:
                    raise UserError(_('Please check "Account Code" of selected Bank Account !!'))

                # lines = payment.move_id.line_ids.with_context(skip_account_move_synchronization=True)
        ...
        if self.check_detail:
            for payment in payments:
                payment.check_detail = self.check_detail
    ...
    # @Add method _create_payments_no_post
    def _create_payments_no_post(self):
        self.ensure_one()
        batches = self._get_batches()
        edit_mode = self.can_edit_wizard and (len(batches[0]['lines']) == 1 or self.group_payment)
    ...
    # @Add method action_create_payments_no_post
    def action_create_payments_no_post(self):
        # payments = self._create_payments()
        payments = self._create_payments_no_post()
