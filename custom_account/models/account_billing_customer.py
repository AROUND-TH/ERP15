...
class AccountBillingCustomer(models.Model):
    _name = 'account.billing.customer'
    ...
    line_ids = fields.One2many('account.billing.customer.line', 'bill_id', 
    ...
    def _compute_name(self):
        ...
        grouped = defaultdict(  # key: bill
            lambda: defaultdict(  # key: first adjacent (date.year, date.month)
                lambda: {
                    'records': self.env['account.billing.customer'],
                    'format': False,
                    'format_values': False,
                    'reset': False
                }
            ...
            for line in bill.line_ids:
                if line.invoice_id:
                    if line.invoice_id.partner_id != partner_id:
                        ...
                        raise Warning(_("Not match with Customer Invoice."))
    ...
    def _compute_payment_state(self):
        for bill in self:
            if bill.state == 'done':
                ...
                receipt_line_ids = self.env['receipt.billing.customer.line'].search([('bill_id', '=', bill.id)])
                receipt_line_ids.filtered(lambda l: l.receipt_id.reconciled_invoices_count == 0)
    ...
    def action_done(self):
        for bill in self:
            ...
            query_chk = """
                ...
                FROM account_billing_customer_line abl
                LEFT JOIN 
                (SELECT abl.bill_id, abl.invoice_id, ab.state 
                    FROM account_billing_customer_line abl
                    INNER JOIN account_billing_customer ab 
    ...
    def button_cancel(self):
        for bill in self:
            if bill.payment_state != 'not_paid':
                raise UserError(_("Cannot cancel 'Billing Note' that 'Payment Status' not in 'Not Paid' status."))
            else:
                bill.state = 'cancel'
                # @Set is_paid = False
                bill.is_paid = False
    ...
    def button_select_invoice(self):
        return {
            ...
            "target": "new",
            "context": {
                'default_customer_bill_id': self.id,
    ...
    def copy(self, default={}):
        default['name'] = "/"
        return super(AccountBillingCustomer, self).copy(default)

    def unlink(self):
        for record in self:
            if record.state == 'done':
                raise UserError(_("Cannot delete 'Billing Note' that 'Status' in 'Done' status."))
        return super(AccountBillingCustomer, self).unlink()



class AccountBillingCustomerLine(models.Model):
    _name = 'account.billing.customer.line'
    ...
    invoice_id = fields.Many2one('account.move', 
        ...
        index=True, 
        required=True, 
        domain="[('partner_id','=',bill_partner_domain),('move_type','=','out_invoice'),('state','=','posted'),('payment_state','=','not_paid')]",
        # check_company=True,
    ...
    #     res = {'domain': {'invoice_id': [('partner_id','=',0),('move_type','=','out_invoice'),('state','=','posted'),('payment_state','=','not_paid')]}}
    #     if self.invoice_id or self.bill_id.partner_id:
    #         res['domain']['invoice_id'] = [('partner_id','=',self.bill_id.partner_id.id),('move_type','=','out_invoice'),('state','=','posted'),('payment_state','=','not_paid')]
    #     return res
