...
class assets_report(models.AbstractModel):
    ...
    def get_header(self, options):
        ...
        return [
            ...
            [
                {'name': ''},  # Description
                {'name': _('Asset No.'), 'class': 'text-center'}, # Characteristics
                {'name': _('Analytic Account'), 'class': 'text-center'},
                {'name': _('Purchase Order'), 'class': 'text-center'},
                {'name': _('Document No.'), 'class': 'text-center'},
        ...
        for company_id, company_asset_lines in groupby(asset_lines, key=lambda x: x['company_id']):
            ...
            for al in parent_lines:
                ...
                line = {
                    ...
                    'columns': [
                        {'name': al['asset_number']},  # Characteristics
                        {'name': al['account_analytic_account_name']},
                        {'name': al['purchase_name']},
                        {'name': al['document_number']},
    ...
    def _get_assets_lines(self, options):
        ...
        sql = """
                ...
                SELECT asset.id as asset_id,
                       ...
                       asset.document_number as document_number,
                       account.code as account_code,
                       account.name as account_name,
                       account.id as account_id,
                       account.company_id as company_id,
                       account_analytic.name as account_analytic_account_name,
                ...
                LEFT JOIN account_analytic_account as account_analytic ON asset.account_analytic_id = account_analytic.id
                LEFT JOIN purchase_order as purchase ON asset.purchase_order_id = purchase.id
                LEFT JOIN (
                    SELECT
                        COUNT(*) as count,
                        asset_id
