...
#                raise UserError(_('You can not delete Purchase Requisition which is not in draft or cancelled or rejected state.'))
    ...
    state = fields.Selection([
        ...
        ('coo_approve', 'Waiting COO Approved'),
        ('ceo_approve', 'Waiting CEO Approved'),
        ('approve', 'Approved'),
    ...
    purchase_receipt_date = fields.Datetime(compute='_compute_purchase_receipt_date', string='Receipt Date')
    ...
    coo_approve_date = fields.Date('COO Approved Date', readonly=True, copy=False)
    ceo_approve_id = fields.Many2one('res.users', string='CEO Approved by', readonly=True, copy=False,)
    ceo_approve_date = fields.Date('CEO Approved Date', readonly=True, copy=False)
    ...
    def _compute_purchase_receipt_date(self):
        for rec in self:
            ...
            for purchase_id in rec.purchase_order_ids:
                purchase_receipt_date = purchase_id.date_planned

            rec.purchase_receipt_date = purchase_receipt_date
    ...
    def action_coo_approve(self):
        for rec in self:
            if self.user_has_groups('material_purchase_requisitions.material_purchase_requisition_coo'):
                rec.state = 'ceo_approve'
                rec.coo_approve_id = self.env.user.id
                rec.coo_approve_date = fields.Date.today()
            else:
                raise UserError(_('Must be approved by the COO only.'))

    def action_ceo_approve(self):
        for rec in self:
            if self.user_has_groups('material_purchase_requisitions.material_purchase_requisition_ceo'):
                rec.state = 'approve'
                rec.ceo_approve_id = self.env.user.id
                rec.ceo_approve_date = fields.Date.today()
            else:
                raise UserError(_('Must be approved by the CEO only.'))
...
#                        raise UserError(_('Select Source location under the picking details.'))
                ...
                stock_id = stock_obj.sudo().with_user(self.create_uid).create(picking_vals)
                delivery_vals = {
...
#                                'company_id':rec.env.user.company_id.id,
                                'company_id':rec.company_id.id,
                                'custom_requisition_id':rec.id,
                                'coo_approve_id':rec.coo_approve_id.id,
                                'coo_approve_date': rec.coo_approve_date,
                                'ceo_approve_id':rec.ceo_approve_id.id,
                                'ceo_approve_date': rec.ceo_approve_date,
                                'origin': rec.name,
                            }
