...
class ImportMRPBom(models.TransientModel):
    ...
    def make_bom_line(self, values, bom_id):
        ...
        mrp_workcenter_obj = self.env['mrp.workcenter'].search([('name','=',values.get('workcenter'))])

        if mrp_workcenter_obj:
            workcenter_id=mrp_workcenter_obj
        else:
            raise ValidationError(_('%s workcenter is not found.') % values.get('workcenter'))
            
        res2 = mrp_routing_workcenter_obj.create({
            'name': values.get('operation'),
            'workcenter_id' : workcenter_id.id,
            'time_cycle_manual' : values.get('time'),
            'bom_id' : bom_id.id,
            })
        return True
