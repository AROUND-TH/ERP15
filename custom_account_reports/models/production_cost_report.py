...
from odoo.addons.custom_account_reports.models.data_item import COLUMNS_PRODUCTION_COST_ITEM

import logging
_logger = logging.getLogger(__name__)

class ReportProductionCost(models.AbstractModel):
    _name = "production.cost.report"
    ...
    def _get_templates(self):
        templates = super(ReportProductionCost, self)._get_templates()
    ...
    def _get_columns(self, options):
        ...
        header2 = [ 
            {'name': '', 'class': 'o_account_coa_column_contrast', 'style': 'width:10%'},
            {'name':_('งบต้นทุนผลิต'), 'class': 'o_account_coa_column_contrast', 'style': 'width:15%'},
            {'name': '', 'class': 'o_account_coa_column_contrast',},
        ]
    ...
    @api.model
    def _get_total_balance(self, options, group_account_ids, type, line_id=None):
        # _logger.info('new_options =========================================== %s' %(options))
        ...
        for account, periods_results in accounts_results:
            ...
            if type in ['A', 'E'] and account.id in group_account_ids.ids and \
            ...
            if type in ['K', 'M'] and account.id in group_account_ids.ids and \
                account.user_type_id.id == self.env.ref('account.data_account_type_current_assets').id:
            ...
            elif type in ['B'] and account.id in group_account_ids.ids and \
                account.user_type_id.id == self.env.ref('account.data_account_type_current_assets').id:
            ...
            elif type in ['H', 'G'] and has_lines and account.id in group_account_ids.ids \
    ...
    def _get_lines(self, options, line_id=None):
        ...
        type_h = [0.0] * (len(options_list))
        type_i = [0.0] * (len(options_list))
        type_j = [0.0] * (len(options_list))
        type_k = [0.0] * (len(options_list))
        type_l = [0.0] * (len(options_list))
        type_m = [0.0] * (len(options_list))
        type_n = [0.0] * (len(options_list))
        ...
        for col in COLUMNS_PRODUCTION_COST_ITEM:
            ...
            for i, option in enumerate(options_list):
                if col['type'] == 'A':
                    ...
                    total_debit, total_credit, total_balance, assets_list = self._get_total_balance(copy_options, group_production_account_ids.group_account_raw_mate_ids, col['type'], line_id)
                    ...
                    total_debit, total_credit, total_balance, assets_list = self._get_total_balance([option], group_production_account_ids.group_account_raw_mate_ids, col['type'], line_id)
                    for j, assets in enumerate(assets_list):

                        columns = [{}] * (len(options_list) + 1)
                        type_b[i] += round(assets['debit'], 2)

                        if i == 0:
                            columns = [
                                {'name': assets['name'], 'class': 'o_account_coa_column_contrast', 'style': 'width:10%'}
                            ] + [
                                {'name': self.format_value(company_currency.round(assets['debit'])), 'class': 'number o_account_coa_column_contrast'}
                            ]

                            lines.append({
                                'id': assets['name'],
                                'name': col['name'] if j == 0 else '',
                                'columns': columns,
                                'class': 'o_account_coa_column_contrast',
                            })
                        else:
                            index = [i for i, line in enumerate(lines) if assets['name'] == line['id']]

                            if index:
                                lines[index[0]]['columns'] += [{'name': self.format_value(company_currency.round(assets['debit'])), 'class': 'number o_account_coa_column_contrast'}]
                            else:
                                lines[index + j]['columns'][i+1] += [{'name': self.format_value(company_currency.round(0.0)), 'class': 'number o_account_coa_column_contrast'}]
                    ...
                    columns[i+1] = {'name':  self.format_value(type_a[i] + type_b[i]), 'class': 'number o_account_coa_column_contrast'}
                    type_d[i] += round((type_a[i] + type_b[i]), 2)

                elif col['type'] == 'E':
                    
                    total_debit, total_credit, total_balance, assets_list = self._get_total_balance([option], group_production_account_ids.group_account_raw_mate_ids, col['type'], line_id)
                    columns[i+1] = {'name': self.format_value(company_currency.round(total_balance)), 'class': 'number o_account_coa_column_contrast'}
                    type_e[i] += round(total_balance, 2)
                
                elif col['type'] == 'F':

                    columns[i+1] = {'name':  self.format_value(type_d[i] - type_e[i]), 'class': 'number o_account_coa_column_contrast'}
                    type_f[i] += round((type_d[i] - type_e[i]), 2)

                elif col['type'] == 'G':
                    
                    total_debit, total_credit, total_balance, assets_list = self._get_total_balance([option], group_production_account_ids.group_account_direct_labor_ids, col['type'], line_id, )
                    columns[i+1] = {'name': self.format_value(company_currency.round(total_balance)), 'class': 'number o_account_coa_column_contrast'}
                    type_h[i] += round(total_balance, 2)

                elif col['type'] == 'H':

                    total_debit, total_credit, total_balance, assets_list = self._get_total_balance([option], group_production_account_ids.group_account_production_ids, col['type'], line_id, )
                    columns[i+1] = {'name': self.format_value(company_currency.round(total_balance)), 'class': 'number o_account_coa_column_contrast'}
                    type_h[i] += round(total_balance, 2)

                elif col['type'] == 'I':

                    columns[i+1] = {'name':  self.format_value(type_h[i] + type_g[i]), 'class': 'number o_account_coa_column_contrast'}
                    type_i[i] += round((type_h[i] + type_g[i]), 2)

                elif col['type'] == 'J':

                    columns[i+1] = {'name':  self.format_value(type_f[i] + type_i[i]), 'class': 'number o_account_coa_column_contrast'}
                    type_j[i] += round((type_f[i] + type_i[i]), 2)

                elif col['type'] == 'K':
                    
                    copy_options = self._copy_options([option])
                    total_debit, total_credit, total_balance, assets_list = self._get_total_balance(copy_options, group_production_account_ids.group_account_start_progress_ids, col['type'], line_id)
                    columns[i+1] = {'name': self.format_value(company_currency.round(total_balance)), 'class': 'number o_account_coa_column_contrast'}
                    type_k[i] += round(total_balance, 2)

                elif col['type'] == 'L':

                    columns[i+1] = {'name':  self.format_value(type_j[i] + type_k[i]), 'class': 'number o_account_coa_column_contrast'}
                    type_l[i] += round((type_j[i] + type_k[i]), 2)

                elif col['type'] == 'M':
                    
                    total_debit, total_credit, total_balance, assets_list = self._get_total_balance([option], group_production_account_ids.group_account_end_progress_ids, col['type'], line_id)
                    columns[i+1] = {'name': self.format_value(company_currency.round(total_balance)), 'class': 'number o_account_coa_column_contrast'}
                    type_m[i] += round(total_balance, 2)

                elif col['type'] == 'N':

                    columns[i+1] = {'name':  self.format_value(type_l[i] - type_m[i]), 'class': 'number o_account_coa_column_contrast'}
                    type_n[i] += round((type_l[i] - type_m[i]), 2)
    ...
    def _get_options_domain(self, options):
        # OVERRIDE
        domain = super(ReportProductionCost, self)._get_options_domain(options)
