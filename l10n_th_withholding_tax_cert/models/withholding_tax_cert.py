...
class WithholdingTaxCert(models.Model):
    ...
    move_id = fields.Many2one(
        ...
        readonly=True,
        states={"draft": [("readonly", False)]},
        domain="[('journal_id.type', 'in', ('general','bank')),"
    ...
    def _onchange_journal_id(self):
        if self.journal_id:
            ...
            return {'domain': {'move_id': [('journal_id.type', 'in', ('general','bank')),('journal_id', '=', self.journal_id.id),('wt_cert_cancel', '=', True), ('state', '=', 'posted')]}}
        else:
            # @This Code is for lock default of selection.
            if self.journal_id_default:
                self.journal_id = self.journal_id_default
                return {'domain': {'move_id': [('journal_id.type', 'in', ('general','bank')),('journal_id', '=', self.journal_id.id),('wt_cert_cancel', '=', True), ('state', '=', 'posted')]}}
            else:
    ...
    #     if self.supplier_partner_id:
    #         self.payment_id = False
    #         self.move_id = False
    #         self.journal_id = False
    #         return {'domain': {'move_id': [('journal_id.type', 'in', ('general','bank')),('partner_id', '=', self.supplier_partner_id.id),('wt_cert_cancel', '=', True), ('state', '=', 'posted')]}}
    #     else:
    #         return {'domain': {'move_id': [('journal_id.type', 'in', ('general','bank')),('wt_cert_cancel', '=', True), ('state', '=', 'posted')]}}
    ...
    def _compute_journal_id_domain(self):
        for rec in self:
            journal_config = self.env['select.journal.config'].search([('form_select', '=', 'withholding_tax_cert')])
            rec.journal_id_domain = journal_config.journal_id

            rec.journal_id_default = False
            if not rec.journal_id and rec.state == 'draft' and journal_config.journal_id:
                journal_id = journal_config.filtered(lambda l: l.set_default == True).journal_id
                if journal_id:
                    rec.journal_id_default = journal_id.id
                ...
                record.update(
                    {
                        ...
                        "name": record.payment_id.name or record.move_id.name or record.name,
                        "date": record.payment_id.date or record.move_id.date or record.date,
    ...
    def action_done(self):
        for rec in self:
            # if not rec.name:
            #     raise ValidationError(_("Cannot Done this document if 'Journal Entry' was not set."))
    ...
    def copy(self, default={}):
        copied_count = self.search_count(
            [('name', '=like', "{}%".format(self.name))])
        if not copied_count:
            new_name = "{}".format(self.name)
        else:
            new_name = "{} ({})".format(self.name, copied_count)

        default['name'] = new_name
        default['date'] = self.date
        default['supplier_partner_id'] = self.supplier_partner_id.id

        return super(WithholdingTaxCert, self).copy(default)
...
class WithholdingTaxCertLine(models.Model):
    ...
    # @Revise for change process of calculate by wt_percent
